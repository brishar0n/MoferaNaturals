    ...
    <div className="mt-5 bg-white shadow-lg rounded-2xl p-6 w-full">
      ...
      <div className="flex flex-col items-center py-5">
          <div className="relative flex items-end">
            <div className="w-32 text-right pr-4 text-xs text-primary font-medium">{formatDate(shipmentData.departure_datetime.split("T")[0])} <br></br>{formatTime(shipmentData.departure_datetime.split("T")[1])}</div>
          ...
          {(packageData.status === 2 || packageData.shipping_id !== null || (checkpointData && checkpointData.length !== 0)) && checkpointData && (
            <div className="relative flex items-end">
              <div className="w-32 text-right pr-4 text-xs text-primary font-medium">{formatDate(checkpointData.arrival_datetime.split("T")[0])} <br></br>{formatTime(checkpointData.arrival_datetime.split("T")[1])}</div>
              <div className="flex flex-col items-center">
          ...
          {(packageData.status === 3 || packageData.shipping_id !== null || packageData.reception_id !== null || (checkpointData && checkpointData.length !== 0) || (receptionData && receptionData.length !== 0)) && checkpointData && receptionData && (
            <div className="relative flex items-end">
              <div className="w-32 text-right pr-4 text-xs text-primary font-medium">{formatDate(receptionData.receival_datetime.split("T")[0])} <br></br>{formatTime(receptionData.receival_datetime.split("T")[1])}</div>
              <div className="flex flex-col items-center">
                  <div className="w-px h-20 border-dashed border border-primary"></div>
                  <div className="w-5 h-5 bg-primary rounded-full z-10"></div>
              </div>
