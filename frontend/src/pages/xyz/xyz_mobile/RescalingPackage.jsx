...
function RescalingPackage() {
    ...
    const [packageNewWeight, setPackageNewWeight] = useState(0)
    const [material, setMaterial] = useState("")

    useEffect(() => {
        async function fetchArrivedPackage() {
            const response = await getPackageByID(parsedPackageId)
    ...
    function handleSubmit () {
        rescalePackage({"id": parsedPackageId, "weight": packageNewWeight, "material": material})
                ...
                <>
                    ...
                    <div className="pb-36 z-10">
                        ...
                        <motion.div
                            ...
                            >
                            <form>
                                <PackageDetails packageData={packageData} packageNewWeight={packageNewWeight} setPackageNewWeight={setPackageNewWeight}/>
                                <MaterialsToCover setMaterial={setMaterial}/>
                                <div className='relative'>
                                    <button className='bg-secondary text-white rounded-3xl px-7 py-2 font-medium hover:bg-primary' onClick={handleSubmit}>Rescale</button>
                                </div>
                            </form>
                        </motion.div>
                    </div>
